...
class UnifiedRandom {
	 ...
	 *
	 * @returns {float} - random float between 0 and 1
	 */
	getValue() {
		...
		for (let i = 0; i < 8; i++) {
			const sign = hash[i];
			const signValue = this._signValues[sign];
			if (i === 0) {
				value = signValue;
			} else {
				let delta = 1;
				for (const _ = 0; _ < i; _++) delta = delta / 16;
	...
	getNumber(min, max) {
		...
		if (isNaN(min) || isNaN(max)) {
			throw new Error(
				"UnifiedRandom error: wrong type of getNumber(min, max) argument(s). They have to be numbers."
			);
		}
